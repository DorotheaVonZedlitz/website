<?php
function fg_job_taxonomy_list( $atts, $content = null ) {

	$atts = shortcode_atts(
		array(
			'title' => 'Hier gehts direkt zu den Jobs:'
		), $atts
	);
	$locations = get_terms( 'jobtypes' );

	if( ! empty( $locations ) && ! is_wp_error( $locations ) ) {
		$displaylist = '<div id="job-location-list">';
		$displaylist .= '<h4>' . esc_html__( $atts[ 'title' ] ) . '</h4>';
		$displaylist .= '<ul class="fg_job_liste">';
...
add_shortcode( 'job_location_list', "fg_job_taxonomy_list" );



function fg_list_job_by_location( $atts, $content = null ) {

	if ( ! isset( $atts['jobtypes'] ) ) {
    ...
    if ( $jobs_by_location-> have_posts() ) :
    	...
    	$location = str_replace( '-', ' ', $atts['jobtypes'] );
    ...
    $display_by_location .= '</div>';
    	...
    	$display_by_location = sprintf( __( '<p class="job-error">Sorry, no jobs listed in %s where found.</p>' ), esc_html__( ucwords( str_replace( '-', ' ', $atts[ 'jobtypes' ] ) ) ) );
    endif;
...
add_shortcode( 'jobs_by_location', 'fg_list_job_by_location' );
